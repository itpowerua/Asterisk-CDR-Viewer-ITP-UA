...
if ( isset($_REQUEST['need_csv']) && $_REQUEST['need_csv'] == 'true' ) {
	...
	echo '<p class="dl_csv_box"><button class="dl_csv btn btn-info" data-filepath="'.base64_encode($csv_fname).'">Завантажити CSV звіт</button></p>';
}
...
if ( isset($_REQUEST['need_html']) && $_REQUEST['need_html'] == 'true' ) {
	...
	if ( isset($tot_calls_raw) && $tot_calls_raw ) {
		...
		try {
			...
			$filtered_count = 0;
			# Видалення дублюючих записів в Asterisk 13
			...
			if ( $tot_calls_raw > $result_limit ) {
				echo '<p class="center title">Відображені '. ($result_limit - $filtered_count) .' із '. $tot_calls_raw;
				echo Config::get('display.main.duphide') == 1 ? ', відфільтровано ' . $filtered_count : '';
				echo ' записів </p><table class="cdr">';
			} else {
				echo '<p class="center title">Знайдено '. $tot_calls_raw;
				echo Config::get('display.main.duphide') == 1 ? ', відфільтровано ' . $filtered_count : '';
				echo ' записів </p><table class="cdr">';
			}
			foreach ( $superresult as $key => $row ) {			
				...
				?>
					<tr>
						<th rowspan="2" class="record_col">Дата та час</th>
						<th rowspan="2" class="record_col">Статус</th>
						<th rowspan="2" class="record_col">Хто дзвонив</th>
						<th rowspan="2" class="record_col">Куди</th>
						<?php
						...
						if ( Config::get('display.column.durwait') == 1
							...
							echo '<th colspan="'.$colspan_duration.'" class="record_col">Тривалість</th>';
						}						
						if ( $use_callrates === true ) {
							...
							if ( Config::exists('display.column.callrates_dst') && Config::get('display.column.callrates_dst') == 1 ) {
								echo '<th rowspan="2" class="record_col">Напрямок</th>';
							}
						}
						if ( Config::exists('display.column.lastapp') && Config::get('display.column.lastapp') == 1 ) {
							echo '<th rowspan="2" class="record_col">Додаток</th>';
						}
...
switch ($group) {
	case "disposition_by_day":
		$graph_col_title = 'Статус по днях';
	...
	case "disposition_by_hour":
		$graph_col_title = 'Статус за годинником';
...
if ( isset($_REQUEST['need_chart']) && $_REQUEST['need_chart'] == 'true' ) {
	...
	if ( $tot_calls ) {
		echo '<p class="center title">Графік по '.$graph_col_title.'</p><table class="cdr">
		<tr>
			<th class="end_col">'. $graph_col_title . '</th>
			<th class="center_col">Усього дзвінків: '. $tot_calls .' | Максимум дзвінків: '. $max_calls .' | Загальна тривалість: '. $tot_duration .'</th>
			<th class="end_col">Сер. час дзвінка</th>
		</tr>';
...
if ( isset($_REQUEST['need_minutes_report']) && $_REQUEST['need_minutes_report'] == 'true' ) {
	...
	echo '<p class="center title">Витрати хвилин по '.$graph_col_title.'</p><table class="cdr">
		<tr>
			<th class="end_col">'. $graph_col_title . '</th>
			<th class="end_col">Кількість дзвінків</th>
			<th class="end_col">Хвилин по білінгу</th>
			<th class="end_col">Сер. час дзвінка</th>
		</tr>';
	...
	echo '<tr>';
	echo "<th class=\"chart_data\">Всього</th><th class=\"chart_data\">$tot_calls</th><th class=\"chart_data\">$html_duration</th><th class=\"chart_data\">$html_duration_avg</th>";
	echo '</tr>';
	echo '</table>';
}
...
if ( isset($_REQUEST['need_chart_cc']) && $_REQUEST['need_chart_cc'] == 'true' ) {
	...
	if ( $tot_calls ) {
		echo '<p class="center title">Паралельні дзвінки по '.$graph_col_title.'</p><table class="cdr">
		<tr>
			<th class="end_col">'. $graph_col_title . '</th>
			<th class="center_col">Усього дзвінків: '. $tot_calls .' | Максимум дзвінків: '. $max_calls .'</th>
			<th class="end_col">Дата дзвінка</th>
		</tr>';
...
if ( isset($_REQUEST['need_asr_report']) && $_REQUEST['need_asr_report'] == 'true' ) {
	...
	echo '<p class="center title">ASR та ACD по '.$graph_col_title.'</p><table class="cdr">
		<tr>
			<th class="end_col">'. $graph_col_title . '</th>
			<th class="end_col">ASR</th>
			<th class="end_col">ACD</th>
			<th class="end_col">Усього дзвінків</th>
			<th class="end_col">Прийнятих дзінків</th>
			<th class="end_col">Секунд по білінгу</th>
		</tr>';
	...
	echo '<tr>';
	echo "<th class=\"chart_data\">Всього</th><th class=\"chart_data\">",($all_asr_total_calls ? intval(($all_asr_answered_calls/$all_asr_total_calls)*100) : 0),"</th><th class=\"chart_data\">",intval($all_asr_bill_secs/($all_asr_answered_calls?$all_asr_answered_calls:1)),"</th><th class=\"chart_data\">$all_asr_total_calls</th><th class=\"chart_data\">$all_asr_answered_calls</th><th class=\"chart_data\">$all_asr_bill_secs</th>";
	echo '</tr>';
	echo '</table>';
