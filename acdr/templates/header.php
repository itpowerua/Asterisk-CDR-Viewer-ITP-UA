...
<script>
	// ID елемента з фоном плеєра
var playerOverlayId = '#playerOverlay',
	// ID елемента з плеєра
	playerId = '#playerBox',
	// Автовідтворення
	...
	// Символ, який буде додано до Title під час відтворення
	playerSymbol = '<?php echo Config::exists('site.js.player_symbol') ? Config::get('site.js.player_symbol') : ''; ?>',
	// Показ стрілок для швидкої навігації праворуч
...
<style>
/* Мінімальна і максимальна ширина всього шаблону */
#container {
		...
		<tr>
			<td id="header_title" colspan="2">
				...
				if ( Config::exists('site.main.main_section') && Config::get('site.main.main_section') != '' ) {
					echo '<span><a title="Перейти в основний розділ" href="' . Config::get('site.main.main_section') . '">&#8592;</a></span>';
				}
		...
		<tr>
			<td class="sub" align='right'>
				...
				if ( strlen(getenv('REMOTE_USER')) ) {
					echo '<a href="index.php?action=logout">Вихід: '.getenv('REMOTE_USER').'</a>';
				}
				?>
			</td>
		</tr>
